...
Le variabili di stato sono le correnti rotoriche nel riferimento dq i_rd e i_rq, e il flusso di statore secondo l'asse diretto.
I forzamenti sono Vsd e vsq, Vrd e Vrq.
L'espressione della coppia è 
$$
\frac{3}{2} \frac{pL_{m}}{L_{s}}\Phi_{sq}i_{sq}
$$
L'ultima volta dicemmo che, guardando la prima equazione, Vsd dipende dalla componente di corrente secondo l'asse diretto. Ciò ci permette di affermare che, se si vuole mantenere il flusso $\Phi_{sd}$ costante, è necessario usare l'equazione (quella rossa) che governa il flusso, e la corrente $i_{rd}$ deve essere pari a $\Phi_{sd}/L_m - L_{sd}/...$
Se ird è pari a zero si ottiene il flusso pari al ??????
Essendo una macchina in cui lo statore è alimentato a frequenza fissa, le componenti di flusso di statore sono a frequenza fissa; dunque si ottiene un flusso $\Phi_{sd}$ anche con corrente ird = 0.
A regime, l'equazione fornisce un valore di flusso anche con ird = 0.

I_rd= 0 si può garantire che il flusso non cambi; se volessi cambiare il valore del flusso e mantenerlo costante, dovrei dare un valore di ird diverso da zero, dato che Vrd è vincolato dalla rete (la parte statorica è collegata direttamente alla tensione di rete).

Si rappresenta tramite queste equazioni lo schema a blocchi; si prova a costruirlo: (vedi xournal)
Come ragionare per costruire lo schema a blocchi? Si vedono le equazioni necessarie da implementare; in teoria ci servono tutte le equazioni, dobbiamo capire quali possono essere utili alla stima della fase del flusso di statore e quali sono utili a imporre i valori di coppia e velocità; stiamo ancora eseguendo un controllo di velocità assoggettato alla richiesta dalla turbina a monte.

Così come svolto nel FOC, il valore di riferimento di velocità proviene dalla caratteristica della turbina che lega la velocità della turbina alla velocità del vento, massimizzando la potenza di generazione (?).
(Inserisci curva potenza meccanica velocità)

Partendo dall'anello più esterno di velocità, cosa si ottiene in uscita dal regolatore di velocità? Si ottiene in uscita dal regolatore la coppia di riferimento e quindi i_rq (come si vede dall'espressione della coppia), avendo imposto il flusso $\Phi_{sd}$ costante.
